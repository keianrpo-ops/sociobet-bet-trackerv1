...
import { X, Save, Edit, DollarSign, AlertTriangle } from 'lucide-react';
...
export const AddBetModal: React.FC<AddBetModalProps> = ({
  ...
  const getDefaultPartnerId = () => partners[0]?.partnerId || '';

  const getEmptyForm = (partnerId: string) => ({
    partnerId,
  ...
  const [formData, setFormData] = useState(() => getEmptyForm(getDefaultPartnerId()));
  const [potentialReturn, setPotentialReturn] = useState(0);

  // ✅ IMPORTANTE:
  // - Solo inicializa/reset cuando el modal se ABRE (isOpen=true)
  // - O cuando cambia initialData (modo edición)
  // - NO depende de "partners" para que la sincronización no pise el formulario mientras escribes
  useEffect(() => {
    if (!isOpen) return;
    ...
    if (initialData) {
      ...
      // Reset SOLO al abrir el modal (no cuando partners se actualiza en background)
      setFormData(getEmptyForm(getDefaultPartnerId()));
    }
  }, [isOpen, initialData]);
            ...
            <div className="bg-rose-50 dark:bg-rose-900/20 p-4 rounded-lg border border-rose-200 dark:border-rose-800 flex items-start gap-3 animate-pulse">
              ...
              <div>
                ...
                <p className="text-xs text-rose-600 dark:text-rose-400 mt-1">
                  El socio seleccionado <strong>{selectedPartner?.name}</strong> aún no ha aceptado el contrato de inversión.
                  No es posible registrar apuestas para este perfil hasta que se regularice su situación.
                </p>
              </div>
            </div>
          )}
