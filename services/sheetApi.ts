...
export const sheetApi = {
  ...
  async updateBet(b: Bet) {
    ...
    const { error, count } = await supabase
      .from('Bets')
      .update(dbBet)
      .eq('betId', dbBet.betId)
      .select('betId', { count: 'exact' });
  ...
  async updateFund(f: Fund) {
    const dbFund = cleanFundForDB(f);
    const { error } = await supabase.from('Funds').update(dbFund).eq('fundId', dbFund.fundId);
    if (error) throw error;
    return { success: true };
  },
  async updateWithdrawal(w: Withdrawal) {
    const dbW = cleanWithdrawalForDB(w);
    const { error } = await supabase.from('Withdrawals').update(dbW).eq('withdrawalId', dbW.withdrawalId);
    if (error) throw error;
    return { success: true };
  },
...
async deleteMessage(messageId: string) {
  const { error } = await supabase.from('Messages').delete().eq('messageId', messageId);
  if (error) throw error;
  return { success: true };
},
};
